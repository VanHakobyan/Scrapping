...
namespace Scrapper
{
    ...
    {
        private const string Link = "https://www.amazon.com/dp/B08FC5L3RG/?coliid=IMO2X4ZPUWGCZ&colid=3AR9V4F4V52IQ&psc=0&ref_=lv_ov_lig_dp_it";
        ...
        public Scrapper()
        {
            _timer.Elapsed += async (sender, args) => { await Parse(); };
            _timer.Start();
        ...
        public async Task Parse()
        {
            try
            {
                ...
                var data = await helper.SendRequestAsync(Link, headers: HeaderBuilder.GetDefaultHeaders(), automaticDecompression: true);
                ...
                {
                    File.AppendAllText(@"D:\playstationLog.log", $"{DateTime.Now}: Not available");
                }
